...
namespace Code.Runtime.Logic.Gameplay
{
    public class Shape : MonoBehaviour, IShapeBase
    {
        ...
        private void OnCollisionEnter2D(Collision2D other)
        {
            ...
            if (other.gameObject.TryGetComponent(out Shape shape))
            {
                if (shape.CombinedCount == 0 && shape.ShapeSize == this.ShapeSize)
                {
                    ...
                    Vector3 spawnPosition = GetSpawnPosition(shape);
                    ...
                    _shapeInteractor.ShapeCombined(this);
        ...
        private Vector3 GetSpawnPosition(Shape shape)
        {
            Vector3 spawnPosition = (this.transform.position + shape.transform.position) / 2f;
